...
%\VignetteDepends{colorspace, RColorBrewer, TeachingDemos, data.table, dendextend, ggplot2, gplots, gridExtra, rpart, rpart.plot, xtable}
...
Local Control (LC) analysis concepts were originally introduced to the \proglang{R} community in 2005, as a suite of functions in the package, ``Unsupervised and Supervised Propensity Scoring in '\proglang{R}' '', or \pkg{USPS} \citep{obenchain2005}. LC is an unsupervised nonparametric approach to adjust for bias in confounder space when comparing a pair of alternative treatments \citep{obenchain2010, obenchain2013}. LC focuses on making ``fair'' comparisons \citep{lopiano2014} using experimental units with confounding characteristics that are as well-matched as possible. Furthermore, LC does not restrict the attention to only treatment ``main-effect'' comparisons; distributions of local effect-sizes are estimated and displayed. Rather than estimating treatment main-effects, LC estimates local effect-sizes within subgroups (clusters) of relatively well-matched patients.

Local Control uses clustering of patients in much the same way that Design-of-Experiments uses blocking \citep{box2005}. Clustering hierarchies are built using confounders believed to be sources of treatment selection bias (Figure \ref{fig:lindHeat}). \fct{localControlClassic} allows users to select the \code{agnes}, \code{diana}, or \code{hclust} method \citep{R, maechler}; choice of clustering algorithm can affect both runtime and the nature of the resulting clusters. After forming the similarity hierarchy, the clustering tree is cut, dividing patients into small or large subgroups, depending on the location of the cut.
\begin{figure}[p]
\centering
<<figure_1, echo=FALSE, fig = TRUE, height=11, width=8.5>>=
set.seed(12345)
...
@
\caption{\label{fig:lindHeat}Lindner hierarchical clustering. On the left is the dendrogram, the tree structure which is cut to produce clusters from the resulting subtrees. The leaves represent individual patients from the data frame. The nine colors at the base of the dendrogram represent the clusters of patients generated by cutting the tree at a relatively coarse granularity. The right side displays a heatmap of the patient covariates where the rows are ordered by the corresponding leaf on the dendrogram.}
\end{figure}
After cutting the tree, each of the resulting clusters is tested for informativeness. Informative clusters contain at least one member of both treatment groups. The local treatment difference (LTD) is the difference in average outcome between the two treatment groups within a cluster.

One of the major exploratory features of Local Control is its interest in controlling the level of patient similarity within clusters. This is done within \fct{LocalControlClassic} by adjusting the height at which the hierarchical clustering trees are cut. Cutting towards the top of a tree results in a small number of large clusters, each containing a relatively broad distribution of confounder values. Cutting toward the bottom of the tree results in more clusters that are smaller in the sense of containing only patients with more similar confounder values. As the number of clusters increases and the size of clusters is reduced, the probability that a cluster will not contain observations from both treatment groups, and hence be uninformative, also increases. This represents a variance-bias trade-off where variability definitely increases as bias is possibly reduced. In the remainder of Section \ref{sec:classicLC}, \fct{LocalControlClassic} is applied to the cardiology data set analyzed in \cite{kereiakes2000}.
%table_1
\subsection{Cross-sectional/case-control data format}\label{sec:csccdata}
\begin{table}[t!]
  \centering
    ...
    \end{tabular}
    \caption{\label{tab:csccData}Cross-sectional/case-control data format. A data frame where one column contains a numerical observed outcome. The treatment column contains a discrete variable with two unique values. At least one of the remaining columns contains pre-treatment covariates used for grouping similar observations.}
\end{table}
Each of the Local Control functions described in this paper require that users provide valid \proglang{R} data frames in order to execute. The data frames must exist in the user's global \proglang{R} environment, prior to calling any of the Local Control functions. While the input requirements vary slightly between cross-sectional and survival analyses, both forms require data frames where the rows contain individual records, while the columns correspond to various patient attributes. The two cross-sectional analysis functions, \fct{LocalControlClassic}, and \code{LocalControl(outcomeType = "cross-sectional")}, require that the input data frame has column vectors corresponding to the following variables:
\begin{itemize}
...
@
Calling \code{LocalControlClassic} returns an \proglang{R} environment containing one \code{UPSnnltd} object (Table \ref{tab:UPSnnltd}) for each value in the list passed to \code{clusterCounts}, as well a summary of the entire analysis. In this example, the \code{linResults} object is an environment containing eleven \code{UPSnnltd} objects. Each \code{UPSnnltd} object is a list containing 34 unique elements, all of which are described in Table \ref{tab:UPSnnltd}. After calling \code{LocalControlClassic}, a useful first impression of the output can be created by plotting statistics describing the distribution of local outcome differences as a function of the number of clusters and fraction of informative patients (Figure \ref{fig:lindLTDs}). This plot is created by passing the returned environment to the \code{LocalControlClassic} plotting function:
...
@
\caption{\label{fig:lindLTDs}\fct{LocalControlClassic} analysis describing the distribution of LTD estimates for the Lindner data set. As the number of clusters is increased, within-cluster patient similarity increases, and less biased estimates of the treatment outcome result. The green line shows the percentage of the patients that fall within informative clusters, which decreases as much smaller clusters are created. Across the spectrum of cluster counts from 15 to 50, the average treatment difference across all clusters is lower than the \$1512 uncorrected estimate. }
\end{figure}
...
hiclus & Name of clustering object created by UPShclus(). \\
  ...
  cumdf & Degrees of-Freedom for the Cumulative Chi-Squared. \\
   \hline
\end{tabular}
\caption{\label{tab:UPSnnltd}Elements of the UPSnnltd object returned from \code{LocalControlClassic}. This function returns a list with one UPSnnltd object for each value in the clusterCounts parameter.}
\end{table}
\clearpage
...
\subsubsection{Data: Cross-sectional/case-control simulation}
\begin{figure}[h]
\centering
...
\begin{figure}[ht]
\centering
<<figure_5, fig = TRUE, eval = TRUE, echo = FALSE, pdf = FALSE, png = FALSE, height=7, width=11, out.width = 9>>=
...
@
\caption{\label{tab:xSimRegOut}Regression output for cross-sim full factorial. A regression is performed to explore the effects of having each variable combination in the model. While weight and dosage are significant, the noise variables are not. This indicates that they should be dropped from the model.}
\end{table}
...
In Section \ref{sec:classicLC}, \fct{LocalControlClassic} was used to analyze the data from the Lindner Abciximab study. Here an analogous analysis is performed using \fct{LocalControl} to provide a comparison of the methods and results. The \fct{LocalControl} function is called using the Lindner dataframe from Section \ref{sec:classicLC}. Confidence intervals are generated using the \code{LocalControlNearestNeighborsConfidence} function. A plot analogous to Figure \ref{fig:lindLTDs} is created, visualizing the average LTD as a function of patient similarity within clusters (radius length). In Figure \ref{fig:lcnnConf}, observe that the \fct{LocalControl} results show a similar reduction in treatment cost as the level of correction increases.

This is done in \proglang{R} with the following commands:\\
\begin{minipage}{\textwidth}
...
\end{table}
This dataset demonstrates the effects of Local Control on correcting bias within survival data. In this simulation, a treatment bias is introduced which skews the global treatment difference. Treatment A is an experimental new treatment which negatively affects patients and leads to an earlier failure time. Treatment B is the trusted alternative, which helps to improve a patient's average survival time. However, the true effects of these drugs are masked by assigning treatment A to younger, lower BMI patients, and treatment B to those who are older and have a higher BMI. That is, the worse drug is given to the healthier patients, making the better drug superficially appear to have inferior outcomes due to the detrimental effects of age and obesity. Table~\ref{tab:ssimData} describes the two treatment groups.
...
The Framingham study data tracks the cardiac health of more than 4000 patients over the course of twenty-four years \citep{framingham}. A subset of the data is provided that has been approved for training and testing purposes. More information about the Framingham Heart Study can be found \href{https://www.framinghamheartstudy.org/}{here}. While the original data includes several additional variables, only the following are used in this analysis:
\begin{itemize}
...
The \proglang{R} \pkg{LocalControl} package has been presented with examples of bias-corrected estimation of treatment outcome differences for case/control, cross-sectional, and time-to event data with competing risks. Patient-level prediction and heterogeneity of treatment effect analysis is currently implemented only for case/control and cross-sectional outcomes. It remains as future work to adapt this approach to survival-based outcomes, for example by extensions to survival-based recursive partitioning trees \citep{bou-hamad2011}.
%\clearpage
